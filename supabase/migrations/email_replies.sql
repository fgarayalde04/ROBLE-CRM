...
-- Se aplica sola al arrancar la app (src/lib/db/migrate.ts).
...
-- Bandeja de respuestas: quién la revisó y cuándo.
alter table email_replies add column if not exists reviewed_at timestamptz;
alter table email_replies add column if not exists reviewed_by text;
