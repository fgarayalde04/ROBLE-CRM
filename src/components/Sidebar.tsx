...
type NavItem = { href: string; label: string; subtitle: string; icon: (p: { className?: string }) => JSX.Element; permission: Permission; badge?: number }
...
export default function Sidebar({ user, isOpen = false, onToggle }: Props) {
  ...
  useEffect(() => {
    ...
    async function fetchUnread() {
      try {
        const res = await fetch('/api/research/unread-count')
        if (!res.ok) return
        const data = await res.json()
        if (!cancelled) setResearchUnread(data.unreadCount ?? 0)
      } catch { /* silent */ }
    }
    fetchUnread()
                  ...
                  <li key={href}>
                    ...
                    >
                      ...
                      <div className="flex flex-col min-w-0 flex-1">
                        <span className="text-[13.5px] font-semibold leading-tight truncate flex items-center gap-1.5" style={{ color: isActive ? '#ffffff' : 'rgba(255,255,255,0.82)' }}>
                          {label}
                          {href === '/research' && researchUnread > 0 && (
                            <span className="shrink-0 text-[10px] font-bold text-[#16A34A]">• {researchUnread}</span>
                          )}
                        </span>
