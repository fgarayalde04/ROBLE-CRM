  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
      <div className={`flex items-center justify-between px-4 py-3 border-b ${c.header}`}>
        ...
        <button type="button" onClick={() => onRemove(id)} className="text-gray-400 hover:text-red-500 transition-colors p-1 rounded" title="Eliminar">
    ...
    <BlockShell title="Renta Variable — Acciones" index={index} id={block.id} color="blue" onRemove={onRemove}>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Operación">
          <select className={selectCls} value={block.operacion} onChange={upd('operacion')}>
            <option value="compra">Compra</option>
            <option value="venta">Venta</option>
          </select>
        </Field>
        <Field label="Nombre de acción">
          <input className={inputCls} placeholder="Ej: Apple Inc." value={block.nombre} onChange={upd('nombre')} />
        </Field>
        <Field label="Ticker">
          <input className={inputCls} placeholder="Ej: AAPL" value={block.ticker} onChange={upd('ticker')} />
        </Field>
        <Field label="Cantidad">
          <div className="flex gap-1.5">
            ...
            <select className="text-sm px-2 py-2 rounded-lg border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-100 shrink-0" value={block.cantidadTipo} onChange={upd('cantidadTipo')}>
              <option value="acciones">acc.</option>
        ...
        <Field label="Moneda">
          <select className={selectCls} value={block.moneda} onChange={upd('moneda')}>
      ...
      <div className="mt-3">
        <Field label="Observaciones">
          <textarea className={inputCls + ' resize-none'} rows={2} placeholder="Instrucciones adicionales…" value={block.observaciones} onChange={upd('observaciones')} />
        </Field>
      </div>
    </BlockShell>
  )
    ...
    <BlockShell title="Fondo de Inversión" index={index} id={block.id} color="emerald" onRemove={onRemove}>
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Nombre del fondo">
          <input className={inputCls} placeholder="Ej: Vanguard S&P 500" value={block.fondo} onChange={upd('fondo')} />
        </Field>
        <Field label="CUSIP / ISIN">
          <input className={inputCls} placeholder="Ej: US9229081090" value={block.cusipIsin} onChange={upd('cusipIsin')} />
        </Field>
        <Field label="Monto">
          <input className={inputCls} type="number" placeholder="Ej: 50000" value={block.monto} onChange={upd('monto')} />
        </Field>
        <Field label="Moneda">
          <select className={selectCls} value={block.moneda} onChange={upd('moneda')}>
            <option value="USD">USD</option>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="col-span-2">
          <Field label="Identificación del bono">
            <input className={inputCls} placeholder="Ej: Uruguay 2031 4.375% — Vto. 15/10/2031" value={block.descripcion} onChange={upd('descripcion')} />
          </Field>
        </div>
        <Field label="CUSIP / ISIN">
          <input className={inputCls} placeholder="Ej: US917288BS29" value={block.cusipIsin} onChange={upd('cusipIsin')} />
        </Field>
        <Field label="Cantidad (Valor Nominal)">
          <input className={inputCls} type="number" placeholder="Ej: 100000" value={block.cantidad} onChange={upd('cantidad')} />
        </Field>
        <Field label="Tipo de precio">
          <select className={selectCls} value={block.precio} onChange={upd('precio')}>
            <option value="mercado">A mercado</option>
...
// ─── Main exported component (embeddable) ─────────────────────────────────────

interface Props {
  gmailConnected: boolean
}

export default function PlantillasClient({ gmailConnected }: Props) {
  ...
  const asunto = `Confirmacion de orden - ${clientNumber || '—'} - ${fecha}`
  ...
  const handleCopy = async () => {
    const text = preview ?? generateEmailText(blocks, clientName, clientNumber, fecha)
  ...
  const handleClear = () => {
    setBlocks([]); setClientName(''); setClientNumber(''); setFecha(todayStr())
    setTo('')
    setPreview(null); setSendStatus(null)
  }

  const handleSend = async () => {
    if (!to.trim()) { setSendStatus({ ok: false, msg: 'Ingresá al menos un destinatario.' }); return }
    const body = preview ?? generateEmailText(blocks, clientName, clientNumber, fecha)
    setSending(true); setSendStatus(null)
    try {
      const res = await fetch('/api/gmail/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ to, cc, subject: asunto, body }),
      })
    ...
    <div className="flex gap-5">
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input className={inputCls} placeholder="Ej: 00123" value={clientNumber} onChange={e => { setClientNumber(e.target.value); setPreview(null) }} />
            </div>
            <div>
              ...
              <div className="flex gap-1.5">
                ...
                <button type="button" onClick={() => { setFecha(todayStr()); setPreview(null) }} className="px-2 py-2 text-xs text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50 shrink-0 transition">Hoy</button>
              </div>
            </div>
          </div>
        </div>
        ...
        {hasBlocks && (
          <div className="space-y-3">
            {blocks.map((block, idx) =>
              block.type === 'acciones' ? (
                <AccionesForm key={block.id} block={block} index={idx} onChange={updateBlock} onRemove={removeBlock} />
              ) : block.type === 'fondos' ? (
                <FondosForm key={block.id} block={block} index={idx} onChange={updateBlock} onRemove={removeBlock} />
              ) : (
        ...
        <div className="bg-white rounded-xl border border-dashed border-gray-300 p-4">
          ...
          <div className="flex gap-2.5">
            <button type="button" onClick={() => addBlock('acciones')} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-blue-200 bg-blue-50 text-blue-700 text-sm font-semibold hover:bg-blue-100 transition">
            ...
            <button type="button" onClick={() => addBlock('fondos')} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 text-sm font-semibold hover:bg-emerald-100 transition">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              Fondos
            </button>
            <button type="button" onClick={() => addBlock('bonos')} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-amber-200 bg-amber-50 text-amber-700 text-sm font-semibold hover:bg-amber-100 transition">
        ...
        <div className="flex items-center gap-2.5">
          <button type="button" onClick={handleGenerate} disabled={!hasBlocks}
            className="px-5 py-2.5 rounded-lg text-sm font-bold text-white disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
          ...
          <button type="button" onClick={handleClear} className="px-4 py-2.5 rounded-lg text-sm font-semibold border border-gray-200 text-gray-500 hover:bg-gray-50 transition">
            Limpiar
          </button>
        </div>
      </div>
      ...
      <div className="w-[380px] shrink-0 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {gmailConnected
              ...
              : <span className="text-[11px] text-amber-600 font-medium">Gmail no conectado</span>
            }
          </div>
          <Field label="Para (To)">
            <EmailAutocomplete
          ...
          <div>
            ...
            <p className="text-sm px-3 py-2 rounded-lg border border-gray-100 bg-gray-50 text-gray-600 truncate">
              {asunto}
            </p>
          </div>
          ...
          <div className="flex gap-2 pt-0.5">
            {gmailConnected && (
              <button type="button" onClick={handleSend} disabled={sending || !hasBlocks}
                className="flex-1 py-2 text-sm font-bold text-white rounded-lg disabled:opacity-40 flex items-center justify-center gap-1.5 transition-opacity"
                style={{ backgroundColor: '#2D3F52' }}>
                {sending
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {preview ? (
            <pre className="p-4 text-xs text-gray-700 font-mono whitespace-pre-wrap leading-relaxed overflow-y-auto max-h-[500px]">
              {preview}
            </pre>
          ) : (
