...
export default function MailPageClient({ googleEmail, sentLogs, gmailConnected }: Props) {
  ...
  useEffect(() => { if (tab === 'bandeja') loadInbox() }, [loadInbox, refreshKey, tab])

  useEffect(() => {
    if (!selected) { setBody(null); setAttachments([]); return }
    setBody(null); setAttachments([])
    setLoadingBody(true); setLoadingAttachments(true)
    fetch(`/api/gmail/message?id=${selected.id}`)
      .then(r => r.json())
      .then(d => { setBody(d.body ?? d.error ?? ''); setAttachments(d.attachments ?? []) })
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Plantillas tab ── */}
      {tab === 'plantillas' && (
        <PlantillasClient gmailConnected={gmailConnected} />
      )}
              ...
              <div className="bg-white border border-[#E2E8F0] rounded-lg overflow-hidden flex flex-col">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center flex-wrap gap-x-2 gap-y-0.5 mt-1.5">
                        <span className="text-[12px] font-medium text-gray-700">{selected.fromName || selected.fromEmail}</span>
                ...
                <div className="px-5 py-5 min-h-[300px]">
                  {loadingBody ? (
                    ...
                    <pre className="text-[13px] text-gray-700 whitespace-pre-wrap font-sans leading-relaxed">{body}</pre>
                  ) : (
              ...
              <>
                ...
                <div className="bg-white border border-[#E2E8F0] rounded-lg overflow-hidden">
                  <div className="px-4 py-3 border-b border-[#EEF0F4] flex items-center gap-2">
                    ...
                    {loadingInbox && <span className="ml-auto text-[10px] text-gray-400 animate-pulse">Cargando…</span>}
                    {!loadingInbox && (
                      <button onClick={() => setRefreshKey(k => k + 1)} className="ml-auto text-gray-400 hover:text-gray-600 transition-colors" title="Actualizar">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  {inboxError && <div className="px-4 py-3 text-xs text-red-600 bg-red-50">{inboxError}</div>}
                  {!loadingInbox && displayMessages.length === 0 && !inboxError && (
                    <div className="px-4 py-12 text-center"><p className="text-sm text-gray-400">No hay emails en la bandeja de hoy.</p></div>
                  )}
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-lg overflow-hidden">
              ...
              >
                ...
                {googleEmail && <span className="text-[10px] text-gray-400 hidden sm:block truncate max-w-[140px]">{googleEmail}</span>}
                <svg className={`w-3.5 h-3.5 text-gray-400 transition-transform ${composeOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-lg overflow-hidden">
              ...
              {(sentLogs ?? []).length === 0 ? (
                <div className="px-4 py-8 text-center"><p className="text-sm text-gray-400">No hay emails enviados.</p></div>
              ) : (
                    ...
                    return (
                      <li key={log.id} className="px-4 py-3 hover:bg-[#F4F6F8] transition-colors">
                        <div className="flex items-baseline justify-between gap-2">
                          ...
                          <span className="text-[10px] text-gray-400 shrink-0">
                            {new Date(log.created_at).toLocaleString('es-UY', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}
                          </span>
                        </div>
